...
class PHUSEBoxPlot:
    ...
    def _filter_analysis_population(self) -> pd.DataFrame:
        ...
        if self.low_ref_var and self.high_ref_var:
            if self.low_ref_var in df.columns and self.high_ref_var in df.columns:
                df['is_outlier'] = (
                    (df[self.measurement_var] < df[self.low_ref_var]) |
    ...
    def generate_box_plots(
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"Generating PHUSE Box Plots")
        ...
        for param_code in param_codes:
            param_data = self.analysis_data[
                self.analysis_data[self.param_code_var] == param_code
            ]
    ...
    def _generate_single_plot(
        ...
        if plot_data:
            ...
            for i, (pos, values) in enumerate(zip(positions, plot_data)):
                ...
                group_data = data[
                    (data[self.visit_num_var] == visit_num) &
                    (data[self.treatment_num_var] == trt_num)
                ]
    ...
    def _add_reference_lines(self, ax, data: pd.DataFrame, ref_lines: str):
        ...
        if ref_lines == 'UNIFORM':
            ...
            if low_vals.nunique() == 1 and high_vals.nunique() == 1:
                ax.axhline(low_vals.iloc[0], color='green', linestyle='--', 
                          linewidth=1, alpha=0.7, label='Lower Ref')
                ax.axhline(high_vals.iloc[0], color='green', linestyle='--', 
                          linewidth=1, alpha=0.7, label='Upper Ref')
        
        elif ref_lines == 'NARROW':
            # Plot narrowest range (max low, min high)
            ax.axhline(low_vals.max(), color='green', linestyle='--', 
                      linewidth=1, alpha=0.7, label='Lower Ref')
            ax.axhline(high_vals.min(), color='green', linestyle='--', 
                      linewidth=1, alpha=0.7, label='Upper Ref')
        
        elif ref_lines == 'ALL':
            # Plot all unique reference lines
            for val in low_vals.unique():
                ax.axhline(val, color='green', linestyle='--', 
