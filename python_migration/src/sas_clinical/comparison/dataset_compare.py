...
class DatasetComparison:
    ...
    def compare_variables(
        self, df1: pd.DataFrame, df2: pd.DataFrame, ds1_name: str = "Dataset 1", ds2_name: str = "Dataset 2"
    ) -> Dict[str, Set[str]]:
        """
        ...
        return {"left": self.left_vars, "right": self.right_vars, "both": self.both_vars}
    ...
    def compare_datasets(
        ...
        if by:
            ...
            # Merge datasets
            merged = df1.merge(df2, on=by_vars, how="outer", suffixes=("_base", "_comp"), indicator=True)
            ...
            if value_diffs:
                ...
                for var, diff_info in value_diffs.items():
                    logger.info(
                        f"  {var}: {diff_info['num_differences']} differences ({diff_info['pct_different']:.2f}%)"
                    )
        ...
        # Summary
        has_diffs = (
            len(self.left_vars) > 0
            or len(self.right_vars) > 0
            or len(df1) != len(df2)
            or len(results.get("value_differences", {})) > 0
        )
        ...
        for ds_name in sorted(common_datasets):
            ...
            try:
                ...
                ds_results = self.compare_datasets(
                    df1, df2, by=by, ds1_name=f"lib1.{ds_name}", ds2_name=f"lib2.{ds_name}", tolerance=tolerance
                )
...
def compare_vars(
    df1: pd.DataFrame, df2: pd.DataFrame, ds1_name: str = "Dataset 1", ds2_name: str = "Dataset 2"
) -> Dict[str, Set[str]]:
    """
...
def compare_datasets(
    df1: pd.DataFrame, df2: pd.DataFrame, by: Optional[Union[str, List[str]]] = None, **kwargs
) -> Dict:
    """
...
def compare_libraries(
    lib1_path: Union[str, Path], lib2_path: Union[str, Path], by: Optional[Union[str, List[str]]] = None, **kwargs
) -> Dict:
    """
